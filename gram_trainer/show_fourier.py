...
"""
Affiche la TF des features dans la base cachée
"""
...
if not os.path.isfile(filename):
    ...
    for i, db in pairings:
        ...
        def implot(X,title=""):
            ...
            plt.title(title)
            plt.show()
        H = network[0](S[0]) # shape : 1x256x120
        G = TF.rfft(H).eval() # shape : 1x256x61x2
        G = G[..., :,0] + G[..., :, 1]*1j
        grams[str(i)] = G
...
for i in range(len(indexes)):
    ...
    plt.figure()
    to_draw = np.clip(np.real(C[indexes[i]][0]), -500, 500)
    plt.imshow(to_draw,cmap=plt.get_cmap('coolwarm'),
            interpolation='nearest', aspect='auto')
    plt.colorbar()
    plt.title(indexes[i] + "-- real")
    #plt.xticks([])
    #plt.yticks([])
    plt.tight_layout()

    plt.figure()
    to_draw = np.imag(C[indexes[i]][0])
    plt.imshow(to_draw,cmap=plt.get_cmap('coolwarm'),
            interpolation='nearest', aspect='auto')
    plt.colorbar()
    plt.title(indexes[i] + "-- imag")
    #plt.xticks([])
    #plt.yticks([])
    plt.tight_layout()
