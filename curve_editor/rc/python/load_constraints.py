#!/usr/bin/env python3
"""
Chargement de la fonction de contrainte
Le calcul arrive plus tard
"""
...
def constrain(X, forward, backward, preprocess, constraint, alpha=0.1, iterations=100):
    ...
    return H, constraint_func

Xrecn = np.expand_dims(Xrecn, axis=0)
Xrecn = np.swapaxes(Xrecn, 1, 2)
H, constraint_func = constrain(Xrecn,
        network_second[0], network_second[1], preprocess,
        multiconstraint(
            foot_sliding(Xtraj[:,-4:]),
            trajectory(Xtraj[:,:3]),
            joint_lengths()),
        alpha=0.01, iterations=250)
